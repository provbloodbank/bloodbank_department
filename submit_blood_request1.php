...
session_start();
include 'connect.php';
// Ensure userid is set in the cookie
if (isset($_COOKIE["patient_id"])) {
    $patient_id = $_COOKIE["patient_id"];
} else {
    echo "<script>alert('Error: patient_id not provided.'); window.location = 'reqbloodpatient.php';</script>"; // Redirect or show an error
    exit();
}
// Ensure userid is set in the cookie
if (isset($_COOKIE["userid"])) {
    ...
    echo "<script>alert('Error: User not logged in.'); window.location = 'homepage.php';</script>"; // Redirect to login
    exit();
}
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO tbl_indication_bt (
        ...
        '$userid', '$patient_id', '$hgb', '$hct', '$red_units', '$platelet_count', '$bleeding_time', '$platelet_units', 
    ...
    if (mysqli_query($con, $sql)) {
        ...
        // Retrieve the bt_id from tbl_patients to join with tbl_blood_types
        $query_patient = "SELECT p.bt_id FROM tbl_patients p WHERE p.patient_id = '$patient_id'";
        ...
        if ($result_patient && mysqli_num_rows($result_patient) > 0) {
            ...
            if ($result_units && mysqli_num_rows($result_units) > 0) {
                ...
                $insert_request = "INSERT INTO tbl_patient_request (patient_id, bt_id, units, request_status, claim_status, claim_date) 
                                    VALUES ('$patient_id', '$bt_id', '$units', 'Pending', 'Not Claimed', NULL)";
            ...
            echo "Error retrieving bt_id from tbl_patients.";
        }
    } else {
...
<body>
    ...
    <section id="content">
        ...
        <main>
            ...
            <hr>
            <form action="submit_blood_request1.php" method="POST" class="blood-request-form">
